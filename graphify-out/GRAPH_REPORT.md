...
## Corpus Check
- 1542 files · ~2,241,305 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 54 file(s) not represented in the graph (top: (none) 16, .bat 5, .css 5)

## Summary
- 32689 nodes · 67243 edges · 1306 communities (1125 shown, 181 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 6629 edges (avg confidence: 0.93)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e1297b4e`
...
## Communities (1306 total, 181 thin omitted)

### Community 0 - "os"
Cohesion: 0.01
Nodes (247): Browser admin UI for login, service control, key management, and diagnostics., ★7 Adaptive Loop Halting — velocity-based agent run termination. Complements…, Cached LLM Client wrapper. Drop-in wrapper around any LLM API call that…, agent/commit_tracker.py — AI Commit Attribution Tags git commits with metadata…, context_pruner.py — auto-generated module docstring (user-research skill scan)., doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, agent/error_interceptor.py — HTTP Error Interceptor Middleware…, agent/harness_enrichment.py — Automatic Harness Enrichment for Agent Prompts… (+239 more)

### Community 1 - "TaskSpec"
Cohesion: 0.01
Nodes (224): abc, 3b. Hermes — **our own** Hermes server (in-repo), UI-wired ✅  (size: M), N2. Surface Hermes (and all runtimes) status in the Doctor/Runtimes UI ⬜  (size: S, risk: low), F2 — MCP Server Exposing Proxy Capabilities [P1] [CBF / ECC], Resolve the model id to force for a code-execution run, or ``None``. Returns…, resolve_coding_model_preference(), kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns… (+216 more)

### Community 2 - "backend/server.py"
Cohesion: 0.01
Nodes (315): set_skill_registry(), admin_seed(), _agent_timeout_fallback_response(), AgentStatusEntry, AgentStatusResponse, AgentToolCallEntry, ApiKeyCreate, auth_me() (+307 more)
...
### Community 4 - "Added"
Cohesion: 0.03
Nodes (123): Public snapshot of live agency state (used by the LiveKit worker tools)., Gather live agency state for SAM's situational awareness., _first_paragraph(), Return the first non-empty, non-heading line. Skips YAML frontmatter (--- ...…, BudgetExceededError, Exception, Reset token counters for all sessions (caps preserved). Called at the start of…, Reset all budgets if the UTC calendar day has changed since last reset. Safe to… (+115 more)

### Community 6 - "Changed"
Cohesion: 0.03
Nodes (172): _fetch_github_quick_notes(), _gh_repo(), Return the GitHub repo in 'owner/name' format. Priority: 1. GITHUB_REPOSITORY…, Return ALL open GitHub issues for this repo (not just 'quick-note' labelled).…, Public wrapper around _generate_plan for callers that want plan-only. Returns…, _fast_count(), Count without materialising rows — prefers estimated_document_count., Changed (+164 more)
...
Cohesion: 0.02
Nodes (157): 8. Backwards compatibility by shim, not by rewrite, Compatibility, heapq, int, FailoverResult, A successful completion plus the accounting its callers need., packages_llm, get_budget() (+149 more)
...
Cohesion: 0.02
Nodes (103): AgentRunner, _run(), _check_extra_kwargs(), _enforce_signature(), Any, Raise TypeError if fn's signature drifts from the locked contract (Pydantic…, Raise TypeError on unknown kwarg (runtime extra='forbid' for non-Pydantic…, Append an event to the durable session log if a store is wired in. (+95 more)

### Community 11 - "brain_config.py"
Cohesion: 0.02
Nodes (125): _migrate_brain_to_safe_default(), Architecture (per plan §3), Files touched, Hard constraints (from the plan) — all met, Implementation — DB-persisted, UI-switchable Brain (PR #824 follow-up), New files, Resolution precedence, Rollout / verification (per plan §5) (+117 more)

### Community 12 - "proxy.py"
Cohesion: 0.03
Nodes (159): Describes a single worker's task and constraints., WorkerSpec, set_quick_note_queue(), get_skill_registry_safe(), Return the global SkillRegistry if set, else None. Used by onboarding and other…, fastapi_middleware_cors, admin_control(), admin_create_key() (+151 more)

### Community 13 - "api.js"
Cohesion: 0.01
Nodes (44): approveGovernanceRequest(), autoRecommendCompanySkills(), createMcpServer(), createQuickNote(), createTask(), delegateSeoFindings(), deleteMcpServer(), deleteModel() (+36 more)

### Community 14 - "[Unreleased]"
Cohesion: 0.03
Nodes (79): Adding a new tool, Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Parse an LLM tool-call response, tolerating common malformations. Handles: 1.…, Condense a tool result down to what belongs in an audit row. A tool result can…, Spawn a child AgentRunner for a delegated sub-task. When sub-agent configs are…, _summarise_tool_result(), build_tool_prompt(), _load_script_module() (+71 more)
...
Cohesion: 0.02
Nodes (154): Added, collections_abc, Added, Modules, Writing a custom adapter, ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, packages/llm/providers/anthropic.py — Anthropic Messages API adapter. Anthropic… (+146 more)

### Community 18 - "_fixture"
Cohesion: 0.02
Nodes (112): Clear pool state (tests only)., reset(), Drop every cached list, attempt record, and dead-model mark. For tests, and for…, reset_cache(), _clear_discovered_models(), _clear_response_cache(), Default all tests to legacy workflow mode (Phase 2 compatibility). Only patches…, Drop the per-provider model listings between tests.… (+104 more)

### Community 19 - "PolicyEngine"
Cohesion: 0.03
Nodes (94): _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, fnmatch, ipaddress, governance_enabled(), True when the governance layer should run at all. A single global off switch…, _action_matches() (+86 more)

### Community 20 - "failover_client.py"
Cohesion: 0.03
Nodes (82): ma(), Fixed, Fixed, _auto_disable(), _build_request(), _describe_registry(), _disable_unless_key_serves_other_models(), _is_ollama() (+74 more)

### Community 21 - "test_operational_incidents.py"
Cohesion: 0.05
Nodes (45): normalise(), Collapse the volatile parts of *message* so recurrences group together. ``Task…, Stable dedup key over the *normalised* message., Find agent phases that started but never finished.…, signature_for(), summarise_phases(), Recurring operational failures must diagnose and file themselves. The gap these…, The reported scenario, end to end: four real timeouts, one incident. (+37 more)

### Community 22 - "resolve_active_brain"
Cohesion: 0.04
Nodes (71): 1. Render (production backend + worker), BrainResolution, provider_api_key(), Return the live API key for *provider* (env-only — never persisted)., get_active_brain_sync(), get_provider_role_tags(), _host_is_openai_compatible(), is_anthropic_model() (+63 more)
...
Cohesion: 0.06
Nodes (58): onboarding_gate_enabled(), Async read of the gate default straight from the DB., Removed, Removed, Removed, Removed, 2. Pending ⬜ — detailed implementation specs, N1. Activate the reliability spine — wire the watchdog, schedule the digest ⬜  (size: M, risk: low) (+50 more)
...
Cohesion: 0.04
Nodes (89): B. Make runtime activation non-blocking (`runtimes/control.py`,, _enrich_runtimes(), get_decision_log(), get_policy(), get_runtime(), list_runtimes(), _load_rich_policy(), PolicyUpdateBody (+81 more)

### Community 28 - "validate_policy_text"
Cohesion: 0.05
Nodes (40): propose_policy(), validate_policy(), packages_governance, _audit_proposal(), _baseline_loosened(), _baseline_patterns(), diff_policy(), _GitHub (+32 more)

### Community 29 - "ExecutionRequest"
Cohesion: 0.02
Nodes (164): contextvars, BoundContext, ClassifyOutput, ExecutionRequest, ExecutionResult, _get_ceo_dispatcher(), get_ceo_fallback_stats(), get_workflow_orchestrator() (+156 more)

### Community 30 - ".apply_diff"
Cohesion: 0.03
Nodes (67): Security surface, Acceptance Checks, `admin_auth.py` checklist, `agent/tools.py` checklist, Escalation, Instructions, `key_store.py` checklist, `proxy.py` auth middleware checklist (+59 more)

### Community 31 - "test_orchestrator_merge_decision.py"
Cohesion: 0.24
...
### Community 35 - "Agency"
Cohesion: 0.04
Nodes (82): Agency, AgencyCycleResult, AgentDirective, AgentRole, _build_ceo_prompt(), _build_quick_note_instruction(), _close_github_issue(), _collect_recent_git_context() (+74 more)

### Community 36 - "Specialist"
Cohesion: 0.03
Nodes (54): What already exists (don't rebuild), SpecialistFamily, Find all specialists of a specific family., Find specialists that can handle a task with given capabilities., Result of provisioning a specialist., A specialist agent that can be provisioned for company-specific tasks., Check if this specialist can handle a task with given capabilities., Specialist (+46 more)

### Community 37 - "Added"
Cohesion: 0.04
Nodes (54): Return True only when the tool is definitively read-only and non-destructive.…, Remember a server-issued ``Mcp-Session-Id`` for subsequent requests., Added, Added, Render MCP — platform debugging and environment monitoring, 1. Coding sessions — stdio, via `.mcp.json`, 2. The running agency — Streamable HTTP against a deployed sidecar, Configuration (+46 more)
...
Cohesion: 0.04
Nodes (62): Backoff policy for retryable failures., RetryConfig, _digest(), KeyRing, KeyState, Per-provider round-robin key selection with cooldowns., Return the next healthy key and its index. Returns ``(None, -1)`` when every…, Cool a key down and return the cooldown length in seconds. Consecutive limits… (+54 more)

### Community 42 - "RenderOpsMonitor"
Cohesion: 0.05
Nodes (31): BaseModel, Response shape of ``GET /api/render/ops/status``. Declared here rather than in…, RenderOpsStatus, One deploy, normalised from whatever shape the tool returned., RenderDeploy, Polls Render for platform-level failures and files them as issues., RenderOpsMonitor, _client() (+23 more)
...
### Community 44 - "probe_report.py"
Cohesion: 0.17
Nodes (18): CreateIssue, build_body(), _failures(), find_tracking_issue(), _gh_ops(), create_issue(), list_issues(), update_issue() (+10 more)

### Community 45 - "yaml"
Cohesion: 0.05
Nodes (24): The representative task catalogue for the routing evaluation. Tasks live in…, merge_step(), Guards on ``.github/workflows/auto-merge.yml``. The job has no checkout step,…, --admin here would merge the major bumps dependabot-auto-merge holds back., test_dependabot_prs_are_left_to_their_own_workflow(), parametrize, Hermes must survive free-tier cold starts, and fallback must not log errors.…, A handled fallback is a warning; only an unrecoverable state is an error. (+16 more)
...
Cohesion: 0.06
Nodes (66): _harvest_changed_files(), Extract the files a runtime touched. Returns ``(files, reported)``. Adapters…, A subtask's full history: what it is, and every attempt at it., SubtaskRecord, Bounds for the unattended supervision loop., SupervisorConfig, _goal(), asyncio (+58 more)
...
### Community 50 - "strategies.py"
Cohesion: 0.04
Nodes (97): count, Preferring local, Choosing one, provider_health, Costs are higher than expected, Latency got worse, HealthConfig, Strategy selection and degradation behaviour. (+89 more)

### Community 51 - "models/company_graph.py"
Cohesion: 0.04
Nodes (92): ApprovalPolicy, BusinessSystem, CompanyCreateRequest, CompanyGraphResponse, CompanyGraphSnapshot, CompanyResponse, CompanyUpdateRequest, Connector (+84 more)

### Community 52 - "resolve_component_model"
Cohesion: 0.03
Nodes (86): _catalog_defaults(), Resolve per-role defaults via the catalog. Returns None on import error., invalidate_brain_config_cache(), Clear the singleton's cache (used by tests + brain_policy invalidation)., Resolve the model id for a component's role on a provider. Parameters…, resolve_component_model(), Checklist, Rollout notes (+78 more)

### Community 53 - "tasks/service.py"
Cohesion: 0.03
Nodes (67): Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., tasks — Task/issue management system. Provides a lightweight task/issue tracker…, ApprovalCheckpoint, ApprovalRequest, ClarifyRequest, _coerce_ts(), CommentAddRequest (+59 more)

### Community 54 - "LLMRouter"
Cohesion: 0.05
Nodes (38): Fixed, Fixed, ErrorFingerprint, Generates stable fingerprints for error deduplication., Create a hash from error type, file, and normalized message pattern., describe(), The operator-facing reason string for an unfixable status., Read the environment variables named in ``env_names`` into a key list. Order is… (+30 more)
...
Cohesion: 0.02
Nodes (128): Added, Added, NVIDIA NIM provider adapter — wraps the existing provider_router logic. This is…, _acquire_provider_probe(), _exponential_backoff_cooldown(), extract_openai_text(), _get_director(), is_commercial_provider() (+120 more)
...
Cohesion: 0.06
Nodes (48): functools, batch_compatibility(), check_model_compatibility(), _detect_amd_gpus(), _detect_apple_silicon_gpu(), _detect_cpu(), detect_hardware(), _detect_intel_arc_gpu() (+40 more)

### Community 64 - "allow_paid"
Cohesion: 0.05
Nodes (38): 1. Mission & operating principles, 2. Brain policy (free cloud LLMs), 3. The Gate Matrix (core artifact), 4. Telegram gate protocol, 7. Definition of "fully autonomous" — acceptance criteria, 8. Safety invariants (carried from `agent/CLAUDE.md`), 🟢 Autonomous — run, then notify-only, Autonomy Charter — Telegram-Gated Self-Running Agency (+30 more)

### Community 65 - "CompanyGraphStore"
Cohesion: 0.02
Nodes (91): bson, BusinessCategory, Added, Added, buildGraphElements(), CompanyGraph, SystemType, Get the index of a phase in the workflow. (+83 more)
...
### Community 67 - "build_tool_calling_router"
Cohesion: 0.24
...
### Community 69 - "test_agent_runner.py"
Cohesion: 0.04
...
### Community 78 - "test_bedrock_provider.py"
Cohesion: 0.06
...
Cohesion: 0.04
Nodes (61): livekit_agents, auth_headers(), livekit_env(), no_livekit_env(), _normalize_dockerfile(), parametrize, tests/test_sam_livekit.py — SAM realtime voice (LiveKit) integration. Covers: -…, Empty key/secret/identity/room must raise ValueError. (+53 more)
...
### Community 85 - "ProvidersScreen.jsx"
Cohesion: 0.06
...
### Community 87 - "UserRole"
Cohesion: 0.06
Nodes (47): Finding A — `list_for_user` Mongo query diverges from the `_can_read` policy, Finding B — `/api/secrets` router is mounted with no authentication dependency, Minor, non-security, Part 1 — The audit, Suggested fixes (not applied here), The chain, What was checked and is sound, str (+39 more)

### Community 88 - "RequestQueue"
Cohesion: 0.04
...
Cohesion: 0.04
Nodes (30): createAgent(), idle, Charts, Donut(), ExecutionTimeline(), Sparkline(), ErrorBoundary, useSafeData() (+22 more)

### Community 94 - "ref_react"
Cohesion: 0.05
Nodes (45): Removed, Removed, getMe(), login(), logout(), App(), AppRoutes(), frontend_src_app_default (+37 more)
...
### Community 99 - "ai/self_heal.py"
Cohesion: 0.05
Nodes (43): Any, packages/ai/self_heal.py — automatic brain self-healing. When the active brain…, Background tick that runs self_heal_brain_and_unblock_tasks(). Called from the…, One-shot self-healing pass. 1. Checks if the active brain provider is in a…, self_heal_brain_and_unblock_tasks(), _self_heal_tick(), _is_provider_actually_available(), Check if a provider is actually available (not just has a key). Unlike… (+35 more)

### Community 100 - "SettingsHub.jsx"
Cohesion: 0.05
...
Cohesion: 0.04
Nodes (68): Ask the C-suite advisory layer a business question. This is the CEO's bridge…, _domain_list_reason(), _domain_matches(), _parse_domain_list(), Return True if *host* is *pattern* or a subdomain of it., Split a comma-separated env var into a normalised domain list., Return why the env-var domain policy blocks *host*, or None. Checks…, Zero-key internet access: pages, YouTube transcripts, search, RSS. Every public… (+60 more)

### Community 111 - "LogWatcher"
Cohesion: 0.06
Nodes (27): _auto_file_enabled(), LogEntry, LogWatcher, A single error entry extracted from a log file., Background daemon that monitors logs and creates GitHub issues for errors.…, Start the log watcher in a daemon thread., Signal the watcher to stop., Force an immediate scan. Returns any errors found (synchronous). Incremental:… (+19 more)
...
Cohesion: 0.09
Nodes (23): engine(), do_approve(), _fake_artifact(), _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Create a WorkflowEngine with isolated storage., TestAbortOnFailure, _test() (+15 more)

### Community 118 - "_StubProvider"
Cohesion: 0.06
Nodes (31): _models_to_try(), Order the models to attempt on *provider*, correcting a stale catalogue. Cache-…, If every candidate is cooling, keep the order rather than sideline the whole…, _mock_get(), _ok(), _handler(), asyncio, parametrize (+23 more)

### Community 119 - "secrets_store.py"
Cohesion: 0.04
Nodes (77): _apply_activity_status(), create_agent(), delete_agent(), get_agent(), _get_user(), list_agents(), list_runtime_agents(), Any (+69 more)

### Community 120 - "get_registry"
Cohesion: 0.05
...
Cohesion: 0.04
Nodes (68): create_access_token(), create_refresh_token(), github_callback(), github_login(), github_repo_access(), github_repo_callback(), github_webhook(), google_callback() (+60 more)

### Community 132 - "gsap.min.js"
Cohesion: 0.05
Nodes (36): ae(), Context(), Db(), Eb(), fb(), Hc(), ia(), Ic() (+28 more)

### Community 133 - "KnowledgeScreen.jsx"
Cohesion: 0.04
Nodes (37): Phase 4 — Onboarding fixes (#593, #619, PR #623) ⏳, createCompany(), createWikiPage(), deleteSource(), ingestSource(), listSpecialists(), updateCompany(), AgentsScreen (+29 more)
...
### Community 135 - "Fixed"
Cohesion: 0.03
Nodes (69): Send chat messages and return the assistant's text output. If an…, Atomically delete a run-once row only while it is still unfired. Closes the…, E2: Classify a failure from its description text. Order matters:…, _autonomy_bg_cycle(), brain_providers(), _in_container(), observability_diag_public(), Called by APScheduler when a cron fires. Dispatches to the orchestrator. This… (+61 more)

### Community 136 - "test_workflow_models.py"
Cohesion: 0.14
Nodes (11): Contract: WorkflowEngine cannot skip the gate state machine., Contract: No code path may advance past awaiting_approval unless gate.status ==…, Create a run and manually place it in awaiting_approval., Contract: Cannot approve a run in 'pending' state., Contract: Can approve a run in 'awaiting_approval' state., Contract: Rejecting a run marks it as failed., TestApprovalGateMandatory, tests/test_workflow_models.py — Unit tests for workflow/models.py. (+3 more)
...
Cohesion: 0.07
Nodes (22): AgentJobManager, get_agent_job_manager(), Public accessor for the module-level AgentJobManager singleton., AgentJobManager method signature enforcement., TestAgentJobManagerContracts, _fake_user(), Path, Return a fixed test UserInfo for a fake user. (+14 more)
...
Cohesion: 0.07
Nodes (16): field_validator, models/seo_audit.py - SEO / GEO / AIO Audit Contracts Typed Pydantic models for…, Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest, mock_report(), _mock_site_handler(), Request, tests/test_seo_audit.py - SEO / GEO / AIO audit engine tests (issue #533).… (+8 more)
...
### Community 157 - "model_router.py"
Cohesion: 0.05
Nodes (40): Adding a model, Adding a task category, CLAUDE.md — router/, Environment variables, Testing, What this package does, Dynamic model router package. Public API:: from router import get_router,…, _build_builtin_model_map() (+32 more)
...
### Community 161 - "user_research_skill.py"
Cohesion: 0.09
Nodes (30): analyze_quantitative(), BaseModel, QualAnalysis, QualQuote, QualTheme, QuantAnalysis, QuantSegment, agent/user_research_skill.py — User research skill for the agent platform.… (+22 more)

### Community 162 - "argparse"
Cohesion: 0.04
Nodes (63): argparse, 1. Authentication & Authorization, API Key Authentication, JWT / Token Auth, Category 2 — API Key Naming Confusion, TD-005 [MEDIUM] — Production Keys Have `test-key-` Prefix, CLI for the cost-aware routing evaluation. python -m evals.cost_aware_routing…, Backward-compatibility shim — use scripts/generate_api_key.py instead. This… (+55 more)

### Community 163 - "services/seo_audit.py"
Cohesion: 0.06
Nodes (43): bs4, csv, io, lxml, BaseModel, A single occurrence of a check firing on a specific URL., Snapshot of one crawled page with the on-page facts the checks used., Site-level facts discovered during the crawl. (+35 more)
...
### Community 166 - "get_task_store"
Cohesion: 0.05
Nodes (67): _purge_backlog_core(), quick_notes_submit(), _QuickNoteBody, Shared purge implementation for the admin endpoint and the boot hook., Submit a quick-note URL or instruction from the dashboard FAB., Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task(), _blocked_retire_age_sec() (+59 more)

### Community 167 - "_resolve_brain_provider"
Cohesion: 0.03
Nodes (66): ProviderUpdate, Decisions (locked with the owner), Design: one UI-controlled Provider Policy (single source of truth), Free NVIDIA brain + UI-controlled provider policy + no silent spend, Implementation plan + TO-DO (check off as you go), Open-PR / issue disposition (read + acted on), Phase 1 — Stop the bleeding + paid kill switch (do first, ship alone if needed), Phase 3 — Persistence hardening (issues #537, #524) (+58 more)
...
### Community 172 - "TestSelfHealingInfrastructureClassification"
Cohesion: 0.05
Nodes (28): filter_safe_tools(), get_tool_annotations(), Typed representation of MCP tool annotations (spec 2025-11-05 §5.6.1). All…, Extract ``ToolAnnotations`` for a named tool from a ``list_tools()`` result.…, Return tools where ``readOnlyHint`` is True and ``destructiveHint`` is not…, ToolAnnotations, FailureCategory, Enum (+20 more)
...
Cohesion: 0.13
Nodes (37): _governance_check(), Evaluate a runtime dispatch against policy; audit it either way. Returns…, _decision(), _engine(), Any, Governance on runtime dispatch — the last coverage gap.…, `route_and_execute` callers handle failure; they do not expect a new exception…, So callers can branch without string-matching the output. Regression: an… (+29 more)
...
Cohesion: 0.03
Nodes (89): A. Connection & credentials, Autonomous SDLC Loop (Agency Core, repo-agnostic), B. Provider & host, C. Delivery / branch policy  *(detected — see DeliveryPolicy)*, Companies without a connected repo (URL-only onboarding), D. CI / checks, Design principle: repo-agnostic, not GitHub-Actions-bound, Detect & respect each repo's delivery policy (+81 more)
...
### Community 186 - "agency_fix.py"
Cohesion: 0.03
Nodes (82): date, build_review_context(), _gh(), main(), Aggregate all review feedback for the PR into a single context string., ``(model_id, label)`` pairs, for callers that log a label., resolve_candidates(), .github/scripts/provider_policy.py — Read the durable provider policy from the… (+74 more)
...
Cohesion: 0.05
Nodes (72): _dispatch_async(), _run(), _ErrorCaptureHandler, get_log_monitor(), LogMonitor, _note_recurrence(), LogRecord, Register the error capture handler on the root logger. (+64 more)

### Community 194 - "_test_e2e_telegram_approval"
Cohesion: 0.12
Nodes (20): _approve_execution_via_rest(), _delete_task(), _extract_admin_token(), _login_admin(), _looks_like_admin_token(), _poll_task_execution_approved(), Any, Client (+12 more)
...
Cohesion: 0.07
Nodes (40): agent, cross_verify(), Any, race(), Heuristic fallback score when the reward model is unavailable.…, Run *n* independent attempts at *instruction* concurrently; return the winner.…, True if any path matches the repo's risky-module trigger list., Have an independent agent re-check a completed task's changed files. Returns… (+32 more)
...
### Community 200 - "operational_incidents.py"
Cohesion: 0.08
Nodes (34): Best-effort: hand an operational failure to the incident tracker. Import is…, _record_operational(), _diagnose_and_file(), _file_incident(), _format_incident(), gather_render_evidence(), get_operational_incident_tracker(), OperationalIncident (+26 more)

### Community 201 - "NotificationDispatcher"
Cohesion: 0.15
Nodes (13): NotificationDispatcher, Routes background task results to configured notification channels. Currently…, Proactively push a Telegram approval-gate message with inline buttons. Sent…, Send a Telegram message with an inline keyboard to all configured chats., FakeTask, Tests for Telegram notification dispatch., Verify NotificationDispatcher.on_task_complete dispatches notifications., TestTelegramNotifications (+5 more)
...
### Community 209 - "chat_handlers.py"
Cohesion: 0.07
Nodes (54): Layer 2 — Chat Handlers (`chat_handlers.py`, 710 lines), Fixed, _apply_chat_defaults(), _apply_reasoning_budget(), _emit_safely(), _extract_exact_output(), _filter_fragment(), _filter_openai_sse_line() (+46 more)
...
Cohesion: 0.06
Nodes (35): Attempt, _backend(), CEOLedger, GoalRecord, _now(), Any, One delegation of one subtask to one tier., One piece of work the CEO has taken ownership of. ``state`` is the supervisor's… (+27 more)
...
### Community 214 - "OperationalIncidentTracker"
Cohesion: 0.10
...
### Community 219 - "traffic_director.py"
Cohesion: 0.08
Nodes (32): provider_max_parallel(), provider_max_rpm(), provider_max_tpm(), _provider_positive_float(), provider_weight(), Shared parse/validate for the numeric per-provider traffic budgets. Returns…, Return the operator-configured requests/min cap for *provider*, or None if…, Return the operator-configured tokens/min cap for *provider*. Reads… (+24 more)

### Community 220 - "AgileManager"
Cohesion: 0.09
...
Cohesion: 0.12
Nodes (38): ArgumentParser, build_parser(), cmd_autostart_install(), cmd_status(), cmd_supervise(), cmd_wait(), Namespace, Block until download completes + colibri answers /v1/models. (+30 more)
...
Cohesion: 0.07
Nodes (27): AgentProfile, Immutable description of a CRISPY agent role. Attributes ---------- role: The…, AgentSwarm, Any, Path, agents/swarm.py — AgentSwarm: routes workflow phases to the correct agent. The…, Return the agent role responsible for *phase*., Return the AgentProfile for the agent driving *phase*. (+19 more)
...
### Community 230 - "audit"
Cohesion: 0.04
Nodes (40): Security, Security, audit(), Request, Append an audit log entry. Never logs raw secrets — only secret IDs / masked…, Go deeper, Quick start, Routes (+32 more)
...
### Community 235 - "handle_anthropic_messages"
Cohesion: 0.13
...
### Community 242 - "openclaw_gateway.py"
Cohesion: 0.08
Nodes (32): _openclaw_instructions(), openclaw_reverse_proxy(), openclaw_status(), openclaw_websocket(), api_route, websocket, Return the OpenClaw Gateway integration status + pairing QR data. The gateway…, Build the instructions string (separate function to avoid .format() brace… (+24 more)
...
### Community 244 - "Autonomy Platform — Remaining Implementation Plan"
Cohesion: 0.11
...
### Community 246 - "reset_store"
Cohesion: 0.04
Nodes (55): Reset the store singleton (used in tests). Also resets the motor client…, reset_store(), Already completed (pre-migration fixes), Current Status, Inventory of suspected dead code, Migration Safety Checklist, Phase 1: Foundation (Weeks 1-2), Phase 2: Provider Abstraction (Weeks 3-4) (+47 more)

### Community 247 - "test_brain_availability_doctor.py"
Cohesion: 0.14
Nodes (24): _doctor(), _P, _patch_providers(), Tests for the public brain-availability diagnosis and the supervisor's use of…, Minimal provider stand-in matching what the summary reads., Wrongly pausing every goal is worse than one wasted re-drive., The remedies differ: one needs a human, the other recovers itself., This is served unauthenticated — no keys, no raw provider error text. (+16 more)
...
Cohesion: 0.03
Nodes (85): gc, AgentScheduler, _now(), Register, list, trigger, and delete cron-scheduled agent jobs. Usage:: sched =…, Register a new job. Returns the created :class:`ScheduledJob`.…, Remove a job. Returns *True* if it existed., Enable or disable a job without deleting it., Return the running event loop, or ``None`` when called synchronously. Used so… (+77 more)
...
### Community 256 - "profiles.py"
Cohesion: 0.16
Nodes (15): agents/__init__.py — CRISPY multi-agent coding system., _catalog_provider(), _get_defaults(), load_all_profiles(), make_architect_profile(), make_coder_profile(), make_reviewer_profile(), make_scout_profile() (+7 more)
...
Cohesion: 0.06
Nodes (36): Changed, Changed, API, changeUserRole(), chatSend(), consultExecutives(), createApiKey(), deleteApiKey() (+28 more)
...
### Community 264 - ".advise"
Cohesion: 0.11
Nodes (11): AdvisoryMemory, Grounding, Any, Evidence gathered for a question before the executives answer., Return the roles whose keywords match *question*. Falls back to a sensible…, Consult the relevant executives and return a unified recommendation. With…, The C-suite's durable memory of past recommendations. A thin adapter over…, get_web_reach() (+3 more)
...
### Community 270 - "generate_context.py"
Cohesion: 0.05
Nodes (62): _check_constitution_echo(), _check_files_exist(), _check_grounding(), _check_hedges(), _check_project_identity(), _check_risk_flags(), _check_source_summary(), _check_todos() (+54 more)
...
### Community 273 - "TestOrchestratorQueue"
Cohesion: 0.13
...
### Community 277 - "test_render_mcp.py"
Cohesion: 0.10
...
Cohesion: 0.04
Nodes (54): packages_ai, clear_cooldowns(), _dead_model_key(), _is_model_dead(), is_provider_on_cooldown(), mark_provider_failed(), Put provider_id on cooldown for *cooldown_seconds* (default:…, Return True if provider_id is currently on cooldown. (+46 more)
...
### Community 288 - "TaskStatus"
Cohesion: 0.03
Nodes (110): _env_flag(), Read a boolean env var. Accepts 'true'/'1'/'yes' (case-insensitive)., Re-queue BLOCKED tasks that have cooled down and are ready for retry., Polls for queued task work and executes it through the coordinator. Crash…, Re-queue tasks stranded by a prior crash or hard-kill., TaskDispatcher, Enum, str (+102 more)

### Community 289 - "CEOSupervisor"
Cohesion: 0.07
Nodes (25): get_ceo_dispatcher(), Return the shared CEODispatcher singleton., Reset the singleton (test helper)., reset_ceo_dispatcher(), CEOSupervisor, Any, What one sweep observed and did. Returned for tests and diagnostics., Sweeps the CEO ledger and drives open goals to closure. (+17 more)
...
### Community 291 - "WorkflowBuildRequest"
Cohesion: 0.07
Nodes (49): TestWorkflowBuildRequest, approve(), build(), cancel(), _engine(), get_agent_team(), get_artifact_content(), get_events() (+41 more)

### Community 292 - "test_e2b_data_flow.py"
Cohesion: 0.06
...
Cohesion: 0.05
Nodes (52): build_block(), _flag(), _int_env(), _known_entry_texts(), _one_line(), propose_entries(), Any, Path (+44 more)
...
### Community 309 - "Dynamic Model Routing"
Cohesion: 0.11
...
Cohesion: 0.09
Nodes (29): get_brain_preference(), Return the operator's brain provider preference. Values: - ``"nvidia"`` —…, Any, Path, run_command(), _admin_out(), register_webui(), _admin_app_index() (+21 more)
...
Cohesion: 0.05
Nodes (21): Company, Any, field_validator, The core company entity - root of the Company Graph., Coerce unrecognised system_type values to 'custom' so the model never crashes…, Get a company by ID. Args: company_id: Company ID Returns: Company instance or…, Update a company. Args: company_id: Company ID **kwargs: Fields to update…, List companies with optional filtering. Args: owner_id: Filter by owner ID… (+13 more)
...
Cohesion: 0.08
Nodes (21): Added, Changed, Fixed, [v4.1.0], Added, Changed, Fixed, [v4.1.0] (+13 more)

### Community 325 - "ImprovementLoop"
Cohesion: 0.05
...
### Community 327 - "test_colibri_provider.py"
Cohesion: 0.13
...
### Community 330 - "is_model_available"
Cohesion: 0.06
Nodes (36): Architecture Audit — local-llm-server, Architecture Diagram, Component Map, Layer 10 — WebUI (`webui/`), Layer 11 — Infrastructure, Layer 1 — API Proxy (`proxy.py`, 1719 lines), Layer 3 — Model Router (`router/`), Layer 4 — Agent System (`agent/`) (+28 more)

### Community 331 - "build_planning_prompt"
Cohesion: 0.16
...
Cohesion: 0.09
Nodes (37): _compose_context(), ExecutiveAdvisory, _format_company_context(), get_executive_advisory(), _keywords(), _mem_key(), agent/executive_advisory.py — an intelligent C-suite business-advisory layer.…, Routes a business question to the right executives and synthesises them. (+29 more)
...
### Community 338 - "note_phase_start"
Cohesion: 0.10
...
### Community 339 - "Kept Rules — the 44 that survive the audit"
Cohesion: 0.05
...
Cohesion: 0.06
Nodes (31): Cerebras provider adapter — free, fast LLM (qwen-3-coder-480b)., GroqProvider, Groq provider adapter — free, fast LLM (deepseek-r1-distill-llama-70b)., Ollama provider adapter — local LLM inference., packages.ai — provider abstraction, model registry, and failover manager., ProviderManager, Any, packages/ai/manager.py — ProviderManager. Single entry point for all LLM calls.… (+23 more)

### Community 344 - "TrafficDirector"
Cohesion: 0.06
Nodes (25): provider_id_of(), Any, Extract a provider id from a ProviderConfig dataclass or a plain dict., In-process traffic distribution and budget accounting for providers., Count a request that was routed away from *provider_id* (diagnostics)., Return *providers* reordered according to the active strategy. ``group_key``…, Reorder one interchangeable group of providers., Ascending sort by *score* with a random tie-break. The tie-break is the point:… (+17 more)
...
Cohesion: 0.10
Nodes (18): Docs / changelog, Frontend changes, Goal, Implementation Plan — Onboarding-Gate Admin Setting + Ephemeral Companies, Requirements → design, Tests, getAccountLifecycle(), getOnboardingSettings() (+10 more)

### Community 354 - "flesch_reading_ease"
Cohesion: 0.20
...
### Community 355 - "Added"
Cohesion: 0.08
Nodes (27): Any, Force the next ``list_tools()`` call to fetch a fresh tool list from the…, Added, Added, A worked example, Adding capacity: multi-key rotation, Attribution, Configuration (+19 more)

### Community 356 - "get_tool_registry"
Cohesion: 0.10
...
### Community 370 - "governance.py"
Cohesion: 0.09
Nodes (28): mcp_server, guard(), identity_from_headers(), _load(), Any, mcp_server/governance.py — governance adapter for the MCP HTTP surface. Closes…, Build an AgentIdentity from the caller's ``X-Agent-*`` headers. Absent headers…, Evaluate *tool* before it runs. Returns ``(allowed, message, decision)``.… (+20 more)

### Community 371 - "capability_registry.py"
Cohesion: 0.10
...
### Community 387 - "test_telegram_approval_gate.py"
Cohesion: 0.13
Nodes (18): Path, Signal the bot to stop and wait for graceful shutdown., Manages the Telegram bot as a managed service alongside ollama/proxy/tunnel.…, TelegramBotManager, _clear_telegram_env(), _FakeResponse, Tests for the TELEGRAM_CHAT_ID single-var convention and the proactive Telegram…, test_send_approval_gate_redacts_goal_and_steps() (+10 more)

### Community 388 - "pytest"
Cohesion: 0.01
Nodes (272): detect_secrets(), main(), Recover CHANGELOG.md from a Git merge conflict in its [Unreleased] block. Pre-…, fastapi_testclient, _load(), main(), ModuleType, Scheduled agile ceremonies digest. Run by `.github/workflows/agile-… (+264 more)
...
### Community 401 - "asyncio"
Cohesion: 0.06
Nodes (23): requires_db, asyncio, Test Company Graph services., Test that storage service can be initialized., Test company CRUD operations - skipped as requires specific config., Test scanner service., Test that scanner service can be initialized., Test specialist service. (+15 more)

### Community 402 - "pr_approval_gate.py"
Cohesion: 0.06
Nodes (55): True if browser automation is enabled and Playwright is importable., merge_pr_route(), PRMergeRequest, N5 dual-auth dependency: accept EITHER a user session OR a service token. Used…, Body for POST /admin/api/prs/{number}/merge — service-token-gated., Merge a PR via the GitHub API. Service-token-gated (N5). Returns the merge…, _user_or_service_token(), Added (+47 more)
...
### Community 409 - "_get_provider_policy"
Cohesion: 0.06
...
Cohesion: 0.14
Nodes (15): _cached(), legacy_scheduler_list(), _produce_scheduler_jobs(), Single-flight TTL cache. Concurrent callers wait for the first producer., _clear_cache(), _CollWithEstimate, _CollWithoutEstimate, asyncio (+7 more)
...
Cohesion: 0.03
Nodes (64): pytest_asyncio, main(), Post-deploy verification for the website scanner against the LIVE internet.…, Returns (url, ok, summary)., _scan_one(), Scan a website and save the results. Args: company_id: Company ID url: Website…, _is_safe_url(), _looks_like_bot_challenge() (+56 more)
...
### Community 428 - "agile_sprints.py"
Cohesion: 0.07
...
### Community 429 - "Testing Analysis — local-llm-server"
Cohesion: 0.13
...
### Community 432 - "Deploy: FreeBuff Telegram bot (24×7)"
Cohesion: 0.09
...
Cohesion: 0.07
Nodes (26): serve_spa(), [5.0.0], Changelog, Changelog, Removed, [5.0.0], Changelog, Changelog (+18 more)

### Community 441 - "test_cerebras_catalogue.py"
Cohesion: 0.13
...
### Community 444 - "test_refresh_agent_built_proof.py"
Cohesion: 0.12
...
### Community 456 - "test_task_source_id_race.py"
Cohesion: 0.14
...
### Community 459 - "test_telegram_service_webhook.py"
Cohesion: 0.19
Nodes (14): _FakeResponse, _make_task(), SimpleNamespace, Regression tests for telegram_service.NotificationDispatcher._notify_webhook.…, Regression: _notify_webhook used to define _send() but never call it., Drain any daemon threads spawned by the dispatcher (webhook/telegram). Snapshot…, Ensure _notify_webhook redacts secrets/PII before sending the webhook payload.…, TestNotifyWebhookActuallyFires (+6 more)

### Community 460 - ".test_thin_static_detection_still_escalates_to_render"
Cohesion: 0.11
...
Cohesion: 0.12
Nodes (19): PreflightIssue, PreflightReport, BaseModel, _fake_user(), test_specialized_runtime_execution(), check_all(), execute(), readiness_check() (+11 more)
...
### Community 469 - "test_memory_guard.py"
Cohesion: 0.12
...
### Community 475 - "test_telegram_mutating_commands.py"
Cohesion: 0.18
...
### Community 480 - "make_isolated_workspace"
Cohesion: 0.16
...
### Community 488 - "test_workflow_orchestrator_scoping.py"
Cohesion: 0.17
...
Cohesion: 0.13
Nodes (20): configure(), get_status(), health(), BaseModel, get, post, Local Service Daemon - Controls services and communicates with GitHub Pages UI.…, Serve the setup wizard UI. (+12 more)
...
Cohesion: 0.07
Nodes (17): get_orchestrator_checkpoint_store(), _NoopDB, OrchestratorCheckpointStore, Any, Restore in-flight runs at startup. Called during backend bootstrap. Returns a…, Fallback in-memory store when no DB is available., Persist orchestrator runs so they survive restarts., Persist a WorkflowRun snapshot. (+9 more)
...
Cohesion: 0.03
Nodes (59): Resolve the JWT signing secret, with a *stable* fallback. Bug fix: the previous…, _resolve_jwt_secret(), Replace the process-wide store. Tests only., reset_approval_store(), AuditEvent, AuditLog, _luhn_ok(), Any (+51 more)
...
### Community 538 - "Autonomy Uplift — Living Roadmap & Detailed Implementation Specs"
Cohesion: 0.17
...
Cohesion: 0.13
Nodes (9): extract_refusal(), is_strict(), Any, Extract the ``refusal`` string from an OpenAI-format response body. Returns the…, Return True when the caller has requested strict schema enforcement. Strict…, is_strict() detects strict: true inside json_schema., extract_refusal() surfaces model refusals from provider response bodies., TestExtractRefusal (+1 more)
...
### Community 550 - "_scanner"
Cohesion: 0.11
Nodes (8): Last-resort fallback that asks builtwith.com what it already knows about a…, Replace curl_cffi's AsyncSession.get with a canned response., The whole point of the headless pass: tech markers that only appear in the JS-…, With rendering enabled but the browser launch failing (CI/sandbox has no…, _scanner(), TestBuiltWithFallback, TestRenderedDomDetection, TestRenderGating
...
Cohesion: 0.03
Nodes (66): Short label for TUI display: {NAME}:{model}, backend, _bootstrap_within_budget(), _create_bootstrap_indexes(), ensure_bootstrap(), The task store the background services should use, wiring it if needed.…, Point the feature stores at the shared database connection. Deliberately…, Create every boot index concurrently rather than one round-trip at a time.… (+58 more)

### Community 553 - ".__init__"
Cohesion: 0.20
...
### Community 593 - "FeatureUnavailableError"
Cohesion: 0.12
...
### Community 598 - "Security"
Cohesion: 0.07
Nodes (44): loops_overview(), Full Loop Engineering fleet view for the UI: the catalogued loops plus the…, seed_default_providers(), _get_cached_tasks(), _get_tasks_cache_lock(), Lock, Return cached task dicts; fetches + caches from TaskStore when stale. Uses…, Security (+36 more)
...
### Community 638 - "test_hermes_server.py"
Cohesion: 0.18
...
### Community 644 - "test_fabric_patterns.py"
Cohesion: 0.10
...
### Community 647 - "Fixed"
Cohesion: 0.04
Nodes (56): Perform MCP handshake. Optional — tools/call works without it., _append_agent_session_message(), Path, _run_agent_loop(), Fixed, Fixed, Active Task Tracker, Bug Log (+48 more)

### Community 648 - "test_crispy_run_history.py"
Cohesion: 0.17
...
### Community 662 - "test_scanner_headless.py"
Cohesion: 0.15
Nodes (9): _is_blocked_host(), Cheap (no-DNS) SSRF check for headless-browser subrequests. A rendered page's…, _guard(), Tests for the scanner's headless-render fallback (JS-rendered / bot-protected…, A scan must never hang past its wall-clock budget — a slow/blocked domain has…, A URL with no hostname (file://, malformed) must be blocked, and…, TestScanBudget, _fast_impl() (+1 more)
...
### Community 682 - "Runbook: Auto-Resume After Cooldown / Interruption"
Cohesion: 0.14
...
### Community 685 - "test_delegate_propagates_runtime_errors"
Cohesion: 0.25
...
### Community 695 - "TestHarnessAdapter"
Cohesion: 0.07
...
### Community 696 - "CircuitState"
Cohesion: 0.18
Nodes (5): CircuitState, Return health snapshots for all known runtimes., Force an immediate health check of all runtimes and return results., Reduce probe frequency for runtimes that have never come online., TestCircuitState
...
Cohesion: 0.06
Nodes (29): Agent roles, AGENTS.md — Codebase Map & Operations Reference, Architecture, Claude Code subagents (cost-aware routing), Codebase map, Deployment, File-size exceptions, Further reading (+21 more)
...
### Community 763 - "467 Public Site Truth Spec"
Cohesion: 0.25
...
Cohesion: 0.07
Nodes (37): Await one warm-up step, deferring it to the background if it overruns.…, _warmup_step(), Added, Fixed, Added, Fixed, AnthropicProvider, Any (+29 more)
...
### Community 776 - "test_log_monitor_storm_guard.py"
Cohesion: 0.48
...
### Community 780 - "review_agent.py"
Cohesion: 0.24
Nodes (6): Example: Access your home PC models from any machine. Install: pip install…, get_pr_diff(), get_pr_files(), load_council_skill(), main(), openai
...
### Community 805 - "CEODispatcher"
Cohesion: 0.04
Nodes (65): A failed drive does not abandon the goal, CEODispatcher, _runner_factory(), CEOResult, _complexity_rank(), _decompose_into_subtasks(), _merge_changed_files(), Any (+57 more)
...
### Community 815 - "test_docker_health_unavailable"
Cohesion: 0.11
Nodes (9): Any, Any, asyncio, test_docker_health_unavailable(), The default RuntimeManager registers only internal_agent + Hermes when…, RUNTIME_CODE_GENERATION=hermes wires a code_generation→Hermes override so a…, test_code_generation_task_type_routes_to_hermes(), test_external_disabled_without_hermes_flag_leaves_only_internal() (+1 more)

### Community 817 - "TestTheCatalogueIsWhatActuallyRuns"
Cohesion: 0.33
...
### Community 824 - "test_setbrain_sends_service_token_header_and_surfaces_success"
Cohesion: 0.20
...
### Community 825 - "SIA.py"
Cohesion: 0.27
Nodes (4): SIA, torch, torch_nn, torch_utils_data

### Community 826 - "test_rate_limiter_concurrency.py"
Cohesion: 0.60
...
### Community 828 - "test_doctor_service_token_check.py"
Cohesion: 0.23
...
### Community 841 - "Part A — CodeRabbit review fixes for this PR (do first, small)"
Cohesion: 0.20
...
### Community 844 - "Request"
Cohesion: 0.27
Nodes (10): middleware, admin_logout(), ollama_api(), openai_compat(), proxy_request(), api_route, Request, Attach X-Request-Id to every response for distributed tracing. Accepts an… (+2 more)
...
### Community 853 - "TestInternalAgentAdapterProviderChain"
Cohesion: 0.40
...
### Community 864 - "TestCallBrainWithFailover"
Cohesion: 0.24
Nodes (5): The regression itself: a 402 (or any non-2xx) must not be fatal while another…, TestCallBrainWithFailover, fake_post(), fake_post(), fake_post()
...
### Community 868 - "lifespan"
Cohesion: 0.07
Nodes (26): _keepalive_self_ping(), lifespan(), _deferred_startup_cleanup(), Run the FreeBuff Telegram bot, restarting it on unexpected exit., Ping our own public URL so a free-tier web service doesn't sleep. Render free…, Start the Telegram bot (and keep-alive) inside the web process when enabled.…, _start_in_web_bot_tasks(), _register() (+18 more)
...
### Community 870 - "_openai_tools_to_anthropic"
Cohesion: 0.50
...
### Community 877 - "_make_mock_response"
Cohesion: 0.24
...
### Community 880 - "test_task_clarification.py"
Cohesion: 0.17
...
### Community 892 - "KnowledgeItem"
Cohesion: 0.06
Nodes (26): Acceptance checks, Fill these in, Skill: client-onboarding, Step 1 — Create the company and kick off onboarding, Step 2 — Poll progress, Step 3 — Verify specialists were provisioned, Step 4 — Confirm the 24x7 agency runtime is live, Step 5 — Note real gaps instead of pretending they're solved (+18 more)
...
### Community 897 - "Direct Chat Evolution: Seamless Assistant Architecture"
Cohesion: 0.40
Nodes (4): Direct Chat Evolution: Seamless Assistant Architecture, Failure Recovery, Overview, Runtime Selection Policy
...
### Community 915 - "test_backend_requirements_cover_runtime_imports.py"
Cohesion: 0.25
...
### Community 920 - "test_scanner_deps_parity.py"
Cohesion: 0.31
...
### Community 924 - "SECTION A — Agent Efficiency (Hermes / AOS / MYT)"
Cohesion: 0.25
...
### Community 928 - "SECTION C — Direct Chat Improvements (CBF / HRM)"
Cohesion: 0.25
...
### Community 952 - ".request_with_retry"
Cohesion: 0.25
...
### Community 971 - "test_doctor_coding_brain.py"
Cohesion: 0.32
...
### Community 973 - "test_generate_context_standing_instructions.py"
Cohesion: 0.32
...
### Community 993 - "test_seo_audit_pending_expiry.py"
Cohesion: 0.43
...
### Community 998 - "Part B — G2: Closed-loop self-heal feedback"
Cohesion: 0.29
...
### Community 1016 - "Next-Pass Roadmap — Detailed Implementation Specs"
Cohesion: 0.33
...
### Community 1018 - "SECTION B — NVIDIA / Cloud Model Integration (Nemotron / NVD)"
Cohesion: 0.33
...
### Community 1026 - "SECTION D — Deployment & Infrastructure (CHM / NVD)"
Cohesion: 0.33
...
### Community 1036 - "Feature Maturity / Support Matrix"
Cohesion: 0.29
...
### Community 1049 - "SkillCategory"
Cohesion: 0.40
Nodes (6): Enum, str, Safety classification for skill execution., Functional category for skill organization., SkillCategory, SkillSafety

### Community 1050 - "_redact_for_notification"
Cohesion: 0.47
...
### Community 1065 - "test_provider_state_durability.py"
Cohesion: 0.25
...
### Community 1067 - "test_runtimes_health_endpoint.py"
Cohesion: 0.33
...
### Community 1068 - "Comparison with OmniRoute"
Cohesion: 0.40
...
### Community 1096 - "steering_for_task"
Cohesion: 0.47
...
### Community 1097 - "apply_phase1_changes.py"
Cohesion: 0.33
...
### Community 1100 - "nvidia_live_test.py"
Cohesion: 0.40
...
### Community 1127 - "_isolate_operator_provider_state"
Cohesion: 0.50
...
### Community 1156 - "_add_colibri_shim_changelog_entry.py"
Cohesion: 0.50
...
### Community 1167 - "get_sam"
Cohesion: 0.67
Nodes (3): get_sam(), get_sam() must return the same instance., test_get_sam_returns_singleton()

### Community 1168 - "SECTION H — Vision / Multimodal (NVD)"
Cohesion: 0.67
...
Cohesion: 0.06
Nodes (42): Changed, Changed, 1.1 Agent Identity, BudgetExceeded, classify(), evaluate_call(), guard_tool_call(), GuardResult (+34 more)
...
- **Why does `AgentRunner` connect `AgentRunner` to `os`, `TaskSpec`, `backend/server.py`, `fmtErr`, `Added`, `test_fabric_patterns.py`, `Changed`, `Fixed`, `Fixed`, `E2BAdapter`, `Path`, `pytest`, `proxy.py`, `[Unreleased]`, `PolicyEngine`, `resolve_active_brain`, `diagnostics.py`, `test_governance_sandbox.py`, `SECTION A — Agent Efficiency (Hermes / AOS / MYT)`, `ExecutionRequest`, `.apply_diff`, `E2BSandboxSession`, `failover_chat_completion`, `Agency`, `ContextManager`, `AgentJobRequest`, `CEODispatcher`, `Added`, `LocalWorkspace`, `AgentPlan`, `test_repo_connection.py`, `Section-by-Section Acceptance Criteria`, `direct_chat.py`, `test_empirical_verify.py`, `CompanyGraphStore`, `enforcement.py`, `test_verification_strategies.py`, `test_agent_runner.py`, `WorkspaceTools`, `FreeBuffAgent`, `test_agent_tool_governance.py`, `StuckDetector`, `UserMemoryStore`, `TestInternalAgentAdapterProviderChain`, `MCPClient`, `ReactScratchpad`, `MultiAgentSwarm`, `state.py`, `test_governance_api.py`, `AgentSessionStore`, `TokenBudget`, `GitHubTools`, `Agency Core — Progress & Resume Log`, `AdaptiveHalter`, `ContextPruner`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `MCPToolResult` connect `AdaptiveHalter` to `os`, `MCPClient`, `Added`, `SEO / GEO / AIO Audit Engine`?**
...
- **Should `os` be split into smaller, more focused modules?**
  _Cohesion score 0.013706102966246164 - nodes in this community are weakly interconnected._
- **Should `TaskSpec` be split into smaller, more focused modules?**
  _Cohesion score 0.01247543709376732 - nodes in this community are weakly interconnected._
- **Should `backend/server.py` be split into smaller, more focused modules?**
  _Cohesion score 0.013278816199376948 - nodes in this community are weakly interconnected._