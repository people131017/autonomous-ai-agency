...
## Corpus Check
- 1529 files · ~2,227,690 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 54 file(s) not represented in the graph (top: (none) 16, .bat 5, .css 5)

## Summary
- 32336 nodes · 60094 edges · 1373 communities (1203 shown, 123 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 4010 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a150a13e`
...
## Communities (1373 total, 123 thin omitted)
...
Cohesion: 0.02
Nodes (174): _env_flag(), Read a boolean env var. Accepts 'true'/'1'/'yes' (case-insensitive)., Helpers that turn scheduler and playbook activity into real tasks., Background dispatcher for task execution., Re-queue BLOCKED tasks that have cooled down and are ready for retry., Polls for queued task work and executes it through the coordinator. Crash…, Re-queue tasks stranded by a prior crash or hard-kill., TaskDispatcher (+166 more)

### Community 2 - "llm/router.py"
Cohesion: 0.02
Nodes (145): int, get_budget(), packages/llm/budget.py — token and cost accounting with spend alerts. Tracks…, The process-wide budget tracker., get_cache(), packages/llm/cache.py — layered caching. Five independent layers, each with its…, The process-wide cache manager., text_key() (+137 more)

### Community 3 - "models/company_graph.py"
Cohesion: 0.02
Nodes (140): ApprovalPolicy, BusinessSystem, CompanyCreateRequest, CompanyGraphSnapshot, CompanyUpdateRequest, Connector, DetectedSystem, Evidence (+132 more)

### Community 4 - "proxy.py"
Cohesion: 0.03
Nodes (170): check_ollama_async(), Attempt a named one-click fix., Async Ollama reachability check — non-blocking for FastAPI handlers., run_fix(), middleware, admin_control(), admin_create_key(), admin_create_user() (+162 more)

### Community 5 - "SQLiteStore"
Cohesion: 0.02
Nodes (92): KnowledgeItem, Structured knowledge about the company, systems, or processes., A company repository with detected technologies and metadata., Extract the owner from the full name., Extract the repository name from the full name., Repo, MongoDBStore, Any (+84 more)
...
Cohesion: 0.02
Nodes (97): base_url(), mobile_page(), proxy_url(), Config, pytest_configure(), conftest.py — pytest fixtures and configuration for the E2E test suite.…, A browser page pre-configured for mobile viewport (390×844 — iPhone 14)., setup_database_moks() (+89 more)

### Community 8 - "brain_config.py"
Cohesion: 0.02
Nodes (121): _migrate_brain_to_safe_default(), _active_primary_provider(), BrainConfig, BrainConfigPatch, BrainConfigStore, default_brain_config(), get_brain_config(), get_brain_config_store() (+113 more)

### Community 9 - "company_api.py"
Cohesion: 0.03
Nodes (134): account_lifecycle(), AccountLifecycleResponse, auto_recommend_skills(), cancel_onboarding(), create_company(), delete_company_endpoint(), _DoctorCheck, _DoctorReport (+126 more)

### Community 10 - "PolicyEngine"
Cohesion: 0.03
Nodes (110): Charge sub-agent nesting depth and enforce the ``max_depth`` ceiling. Isolates…, _egress_policy_reason(), agent/web_reach.py — Web Reach: zero-key internet access for agents. Gives…, Return why governance policy blocks *host*, or None. Runs strictly *after* the…, simulate_policy(), guard(), _load(), mcp_server/governance.py — governance adapter for the MCP HTTP surface. Closes… (+102 more)

### Community 11 - "LLMRequest"
Cohesion: 0.03
Nodes (108): ProviderConfig, One configured endpoint. ``kind`` selects the adapter: ``openai`` (any OpenAI-…, build_provider(), Normalise a configured ``kind`` to an adapter key., Instantiate the adapter for one provider config. Unknown kinds fall back to the…, resolve_kind(), LLMRequest, Cheap character-based token estimate (~4 chars/token). Deliberately dependency-… (+100 more)
...
Cohesion: 0.02
Nodes (93): Test hook — clear all usage accounting., reset(), cosine_similarity(), payload_key(), Exact-match cache key over the fields that change the answer. Routing…, Cosine similarity between two vectors, 0.0 when either is degenerate., Test hook — drop every cache layer., reset() (+85 more)

### Community 14 - "Task"
Cohesion: 0.03
Nodes (88): Full task/issue document., Task, _gate_outward_facing_enabled(), _is_brain_connection_error(), _is_outward_facing(), Any, BaseException, TaskSpec (+80 more)

### Community 15 - "RuntimeHealth"
Cohesion: 0.04
Nodes (74): kimi_bridge_runtime_config(), Return Kimi bridge config for external runtimes (Hermes, Goose, Aider). Returns…, AiderAdapter, runtimes/adapters/aider.py — Aider adapter (TIER 3 — specialized). Aider…, Adapter for Aider — TIER 3 specialized git-aware code editor., runtimes/adapters/claude_code.py — Claude Code CLI adapter (FIRST CLASS).…, DockerAgentAdapter, runtimes/adapters/docker_agent.py — Docker-based agent runtime adapter. Spawns… (+66 more)
...
Cohesion: 0.03
Nodes (106): allow_paid_brain(), BrainResolution, get_brain_preference(), get_provider_role_tags(), _host_is_openai_compatible(), invalidate_brain_cache(), _norm(), _pick_from_records() (+98 more)
...
Cohesion: 0.03
Nodes (62): get_tool_registry(), Return the module-level ToolRegistry singleton. On first call, registers built-…, AgentRunner, _run(), Any, Path, Governance seam: judge the call, run it, audit the outcome. Deliberately a thin…, Append an event to the durable session log if a store is wired in. (+54 more)
...
Cohesion: 0.04
Nodes (74): MCPUnavailableError, Raised when the MCP server is unreachable or the circuit is open., TaskResult, Execute a task inside a fresh E2B sandbox. Flow: 1. Open an…, Run ``pytest`` inside the sandbox. Returns ``(output, passed)``.…, E2BSandboxSession, _inject_token(), maybe_attach_e2b() (+66 more)

### Community 23 - "test_failover_client_shared.py"
Cohesion: 0.05
Nodes (95): BrainFailoverExhausted, failover_chat_completion(), FailoverResult, _log_recovery(), RuntimeError, Every provider in the failover chain failed — the terminal error. Carries the…, A successful completion plus the accounting its callers need., Report a successful failover at INFO — recovery is not an incident. Everything… (+87 more)

### Community 24 - "TaskWorkflowService"
Cohesion: 0.04
Nodes (62): AgentDefinition, AgentStore, Any, field_validator, agents/store.py — Persistent store for user-defined agent configurations.…, CRUD store for AgentDefinition objects. Uses MongoDB when a `db` client is…, Return an agent by ID. If *owner_id* is provided, enforces that the agent…, Delete an agent. Returns True on success, False if not found/unauthorised. (+54 more)
...
Cohesion: 0.04
Nodes (40): Raised when a runtime fails readiness validation before execution starts., RuntimePreflightError, Any, Return health status for all registered runtimes., Return routing decision audit log (newest first)., Actively wake every sleeping/circuit-open runtime. The default health service…, True if the runtime is healthy enough for the router to select it., Return cached health snapshot for a runtime (sync, non-blocking). (+32 more)
...
Cohesion: 0.04
Nodes (55): agent/job_manager.py — Async agent job lifecycle manager. Manages agent jobs…, TaskResult, Run aider non-interactively via `--message` flag., TaskResult, TaskResult, TaskResult, _best_cloud_primary_base(), TaskResult (+47 more)
...
### Community 33 - "get"
Cohesion: 0.02
Nodes (88): auth_me(), auto_recommend_skills(), _check_storage_health(), cost_attribution_stats(), discover_remote_skills(), doctor_health(), get_catalog_models_route(), get_public_kpis() (+80 more)
...
### Community 35 - "brain_failover.py"
Cohesion: 0.04
Nodes (80): auto_disable_provider(), BrainFailoverManager, _disabled_from_mongo(), _disabled_from_sqlite(), _is_paid_allowed_db(), _kv_connect(), _kv_path(), _mongo_db() (+72 more)

### Community 36 - "test_ceo_dispatcher.py"
Cohesion: 0.03
Nodes (86): _decompose_into_subtasks(), _merge_changed_files(), Decompose a request into 2-3 sub-tasks for specialist fan-out. Default…, Collect changed_files across all specialists into a single de-duped list., get_ceo_fallback_stats(), PlanOutput, PLAN phase: structured execution plan., SELECT_SPECIALIST phase: which specialist(s) handle this work. (+78 more)
...
Cohesion: 0.04
Nodes (74): all_provider_ids(), _provider_ids_from_literal(), Resolve the model id for a component's role on a provider. Parameters…, Return every provider id recognised by the brain config system. Iterates the…, Return the provider ids allowed by the ``BrainProvider`` Literal. Reads the…, resolve_component_model(), CatalogActiveBrain, CatalogMirror (+66 more)
...
Cohesion: 0.04
Nodes (53): plan_next_sprint(), The result of allocating portfolio capacity into a new sprint., Allocate ``capacity`` of WSJF-ranked initiatives into a new sprint. Creates one…, SprintPlan, Initiative, Enum, Agentic Portfolio Management — initiative prioritisation, capacity allocation,…, Roadmap placement for an initiative (Now/Next/Later planning). (+45 more)
...
Cohesion: 0.04
Nodes (44): AgentJob, AgentJobManager, heartbeat(), _now(), Any, Run a job using the provided runner and update the job's lifecycle, progress,…, Serialize the AgentJob to a JSON-serializable dictionary for external clients.…, AgentJobManager method signature enforcement. (+36 more)
...
Cohesion: 0.05
Nodes (74): BudgetTracker, GovernanceGate, Holds live session budgets, bounded so it cannot leak. Sessions end without…, The one seam through which governed actions pass., AgentIdentity, _coerce_text(), new_session_id(), Any (+66 more)

### Community 46 - "direct_chat.py"
Cohesion: 0.05
Nodes (78): Any, Translate technical preflight issues into a conversational assistant reply., translate_error_to_conversational(), classify_direct_chat_intent(), _contains_keyword(), detect_intent(), intent.py — Intent classification for direct chat (answer_only, execute_now,…, Return True if content contains any execution or analysis keyword. (+70 more)
...
### Community 54 - "test_llm_router_gateway.py"
Cohesion: 0.04
Nodes (45): health(), metrics_json(), toggle_provider(), get_tracker(), The process-wide health tracker., _Counter, _escape(), _Gauge (+37 more)

### Community 55 - "telegram_bot.py"
Cohesion: 0.05
Nodes (77): get_decisions_store(), Process-wide DecisionsStore singleton (resettable via db_path arg)., classify_plain_text(), services/inbound_router.py Helpers for the Telegram inbound router. Three small…, True when the message exceeds the delivered-character budget. Uses…, Write ``text`` to a deterministic paste file and return its path. Writes under…, Return a Markdown-v1-safe preview string under ``max_chars``. Used by the…, Classify plain-text into one of the direct-chat intent categories. Re-uses… (+69 more)

### Community 56 - "validate_policy_text"
Cohesion: 0.04
Nodes (40): propose_policy(), validate_policy(), _audit_proposal(), _baseline_loosened(), _baseline_patterns(), diff_policy(), _GitHub, parse_policy_text() (+32 more)
...
Cohesion: 0.05
Nodes (50): field_validator, A single occurrence of a check firing on a specific URL., Site-level facts discovered during the crawl., Request to run an SEO/GEO/AIO audit against a website., SeoAuditRequest, SeoIssueInstance, SeoSiteFindings, fire() (+42 more)

### Community 59 - "LLMRouter"
Cohesion: 0.05
Nodes (49): Attempt, Read the environment variables named in ``env_names`` into a key list. Order is…, resolve_keys(), LLMRouter, run(), Any, AsyncClient, BaseException (+41 more)

### Community 60 - "AnthropicProvider"
Cohesion: 0.05
...
### Community 61 - "ModelConfig"
Cohesion: 0.04
Nodes (40): AgentPolicy, LLMConfig, ModelConfig, Capability and pricing metadata for one model id., Per-agent routing overrides (ADR-008 §"named profiles")., The fully-resolved routing configuration., Re-read configuration from disk. Used by the admin API and by tests., reload_config() (+32 more)

### Community 62 - "CompanyGraphService"
Cohesion: 0.03
Nodes (44): BusinessCategory, A company website with detected systems and stack inference., Website, requires_db, CompanyGraphService, Company, SpecialistFamily, SystemType (+36 more)
...
Cohesion: 0.05
Nodes (65): _auto_disable(), _describe_registry(), _disable_unless_key_serves_other_models(), _disabled_ids(), _is_billing_refusal(), _is_ollama(), _key_pool(), _log_exhaustion() (+57 more)
...
Cohesion: 0.05
Nodes (54): Weighted Shortest Job First score — higher schedules sooner., _purge_backlog_core(), quick_notes_submit(), _QuickNoteBody, Shared purge implementation for the admin endpoint and the boot hook., Submit a quick-note URL or instruction from the dashboard FAB., Hand the 'connect & verify the repo' work to the agency's own agents. The task…, _seed_connect_task() (+46 more)

### Community 77 - "CEOSupervisor"
Cohesion: 0.05
...
### Community 79 - "test_sam_livekit.py"
Cohesion: 0.04
Nodes (57): auth_headers(), livekit_env(), no_livekit_env(), _normalize_dockerfile(), tests/test_sam_livekit.py — SAM realtime voice (LiveKit) integration. Covers: -…, SAM_LLM_* env vars must override the NVIDIA defaults (Hermes/proxy routing)., Under TESTING the in-process worker must never be eligible to start., OPT-IN: defaulting to on OOM-killed the 512MB Render instance at boot… (+49 more)
...
### Community 85 - "test_autonomous_agency_e2e.py"
Cohesion: 0.06
...
Cohesion: 0.05
Nodes (40): clear_cooldowns(), CommercialFallbackRequiredError, get_dead_models(), _is_model_dead(), is_provider_on_cooldown(), mark_provider_failed(), ProviderAttempt, ProviderFallbackError (+32 more)
...
### Community 105 - "CEODispatcher"
Cohesion: 0.06
Nodes (41): CEODispatcher, _runner_factory(), _complexity_rank(), _offload(), Any, Semaphore, services/ceo_dispatcher.py — Real CEO delegation layer. The CEO splits a…, Run a synchronous ledger call without blocking the event loop. ``CEOLedger`` is… (+33 more)
...
### Community 132 - "probe_model_liveness"
Cohesion: 0.05
...
Cohesion: 0.06
Nodes (37): engine(), WorkflowEngine, Contract: WorkflowEngine cannot skip the gate state machine., Contract: No code path may advance past awaiting_approval unless gate.status ==…, Create a run and manually place it in awaiting_approval., Contract: Cannot approve a run in 'pending' state., Contract: Can approve a run in 'awaiting_approval' state., Contract: Rejecting a run marks it as failed. (+29 more)
...
Cohesion: 0.06
Nodes (39): is_commercial_provider(), provider_access_tier(), _provider_field(), provider_sort_key(), ProviderConfig, The pieces the failover loop assembles for an Anthropic-native provider., TestAnthropicRequestShape, Tests that verify the exact provider priority ordering. (+31 more)

### Community 139 - "TestAdapterMetadata"
Cohesion: 0.05
...
Cohesion: 0.07
Nodes (52): Reset the singleton (for tests)., reset_failover_manager(), _blocked_retire_age_sec(), _heal_blocked_backlog(), _heal_brain_failover(), _heal_purge_backlog(), _heal_stuck_tasks(), _heal_telegram() (+44 more)

### Community 141 - "test_issue_intake.py"
Cohesion: 0.07
Nodes (50): _heal_task_duplicates(), Backfill source_id on legacy ceo_direct tasks, then dedup by source_id. Two…, _capability_tags(), create_task_from_oldest_open_issue(), intake_issue(), _issue_labels(), issue_source_id(), map_issue_to_task() (+42 more)
...
Cohesion: 0.06
Nodes (28): _fake_artifact(), _make_engine(), tests/test_crispy_workflow.py — CRISPY workflow engine hardening tests. Tests…, Provide isolated DB + artifact + workspace paths., Create a WorkflowEngine with isolated storage., TestAbortOnFailure, _test(), _tracking_run_single() (+20 more)

### Community 149 - "test_kimi_bridge_server.py"
Cohesion: 0.05
...
### Community 160 - "skill_bindings.py"
Cohesion: 0.06
...
Cohesion: 0.06
Nodes (36): Event, publish(), packages/events/bus.py — In-process event bus. Loosely couples components via…, An event published on the bus., Subscribe to an event type., Publish an event to all subscribers., subscribe(), ApprovalRequest (+28 more)
...
### Community 179 - "workflow_orchestrator.py"
Cohesion: 0.09
Nodes (39): BoundContext, ClassifyOutput, ExecutionResult, JudgeVerdict, MergeDecision, MonitorOutput, _orchestrator_bypass(), PersistOutput (+31 more)
...
Cohesion: 0.08
Nodes (35): _build_anthropic_response(), _emit_safely(), _finish_reason_to_stop_reason(), get_local_model(), handle_anthropic_messages(), _messages_to_openai(), _openai_choice_to_anthropic_content(), _post_anthropic_with_fallback() (+27 more)
...
Cohesion: 0.07
Nodes (29): _build_pdf(), findings_block(), hr(), S(), section(), main(), _parse_args(), Namespace (+21 more)
...
Cohesion: 0.06
Nodes (26): _note_recurrence(), _parse_timestamp(), datetime, Parse an RFC3339 timestamp from Render, tolerating a trailing ``Z``., Run one full scan and return every finding, filed or not. Never raises: a…, Scan, then file everything that is past its cooldown. The cooldown is claimed…, Map service ID → human name, degrading to the ID when unavailable., Flag a failed latest deploy, or one that has been building too long. (+18 more)
...
### Community 237 - "models/seo_audit.py"
Cohesion: 0.10
Nodes (21): BaseModel, models/seo_audit.py - SEO / GEO / AIO Audit Contracts Typed Pydantic models for…, Snapshot of one crawled page with the on-page facts the checks used., Lightweight listing entry for past audits., Request to remediate auto-fixable findings in a local code repository., One concrete remediation performed (or proposed) by the fixer., Result of a fixer run., SeoAuditSummary (+13 more)
...
### Community 242 - "self_healing.py"
Cohesion: 0.08
Nodes (27): FailureCategory, heal_signature(), HealState, Enum, str, agent/self_healing.py — Self-Healing Agent (closed-loop, Autonomy Charter G2)…, Classified failure types for targeted self-healing (E2)., Lifecycle of a heal (Autonomy Charter G2 closed loop). (+19 more)

### Community 243 - "lifespan"
Cohesion: 0.06
Nodes (25): _keepalive_self_ping(), lifespan(), _deferred_startup_cleanup(), Run the FreeBuff Telegram bot, restarting it on unexpected exit., Ping our own public URL so a free-tier web service doesn't sleep. Render free…, Start the Telegram bot (and keep-alive) inside the web process when enabled.…, #522 + #505: Reliability startup — schedule hydration, orchestrator restore,…, _start_in_web_bot_tasks() (+17 more)
...
Cohesion: 0.07
Nodes (25): CompanyAgencyService, _is_runtime_available_sync(), _pick_available_runtime(), Any, SpecialistFamily, services/company_agency.py — Company Agency Orchestration Service After…, Orchestrates specialist activation, runtime startup, and 24x7 scheduling for a…, Return the best available runtime for a specialist family. Checks available… (+17 more)
...
Cohesion: 0.08
Nodes (34): get_sam(), agent/sam.py — SAM Voice Agent (System Autonomy Manager) SAM is the voice-…, autonomy_tick(), _get_backend_sam(), legacy_scheduler_delete(), legacy_scheduler_get(), Lazy-init the SAM agent for the backend deployment., Get SAM's current status. (+26 more)
...
### Community 269 - "ai/router.py"
Cohesion: 0.08
Nodes (32): _acquire_provider_probe(), _dead_model_key(), _exponential_backoff_cooldown(), _get_director(), _mark_model_dead(), _notify_watchdog(), ProviderResult, provider_router.py — auto-generated module docstring (user-research skill scan). (+24 more)
...
### Community 273 - "test_agent_coordinate.py"
Cohesion: 0.11
...
### Community 276 - "test_backend_server_features.py"
Cohesion: 0.07
...
### Community 284 - "PreflightReport"
Cohesion: 0.08
Nodes (20): PreflightIssue, PreflightReport, BaseModel, doctor.py — Agent-side doctor diagnostics: environment, provider, and workspace…, Regression: AgentRunner must not be called with removed kwargs. Previous…, test_agent_runner_no_stale_kwargs(), check_all(), test_sticky_objective_memory() (+12 more)
...
### Community 304 - "RuntimeReadinessReport"
Cohesion: 0.08
Nodes (17): Fail preflight when the provider extension is configured but missing. Without…, Any, Structured, actionable preflight validation issue., Preflight result returned before a runtime task starts., Serialise adapter metadata (no secrets)., RuntimeReadinessReport, RuntimeValidationIssue, _fake_user() (+9 more)
...
### Community 312 - "provider_max_rpm"
Cohesion: 0.09
...
### Community 352 - "PolicyDecision"
Cohesion: 0.11
Nodes (19): guard_tool_call(), GuardResult, Any, The outcome of guarding one action., A message safe to hand back to the model as a tool result. Names the rule so…, Evaluate, gate, and (if required) hold for approval. Returns a…, Create an approval request and wait for its verdict., Audit the *outcome* of an allowed action and charge its cost. Split from… (+11 more)
...
### Community 360 - "test_ceo_router.py"
Cohesion: 0.13
Nodes (27): Drop the singleton (test helper)., reset_ceo_ledger(), ledger(), _make_app(), _fake_get_current_user(), FastAPI, tests/test_ceo_router.py — auth and behaviour for /api/ceo/*. Follows the…, The happy path — the branch that actually spends provider budget. (+19 more)

### Community 361 - "SeoFixer"
Cohesion: 0.15
...
### Community 387 - "TestNoNvidiaFallbackIsRetired"
Cohesion: 0.09
...
### Community 390 - "trend_watcher.py"
Cohesion: 0.14
...
### Community 392 - "unsafe_target_reason"
Cohesion: 0.12
...
### Community 415 - "has_image_content"
Cohesion: 0.12
...
### Community 424 - "test_repairs_when_complete_but_zero_specialists"
Cohesion: 0.09
...
### Community 443 - "_mock_provider_records"
Cohesion: 0.12
...
Cohesion: 0.10
Nodes (15): _escape_md_v1(), service_manager.py — Telegram & Notification Integration Extension Extends the…, True when outbound Telegram sends must be suppressed. Tests must never page a…, Proactively push a Telegram approval-gate message with inline buttons. Sent…, # NOTE: do NOT re-escape here — services.daily_digest.format_digest_markdown, Escape Telegram Markdown-v1 reserved chars in free-text fields. Markdown-v1…, _telegram_sends_suppressed(), test_send_guard_holds_under_pytest() (+7 more)
...
### Community 463 - "test_compare_runtimes.py"
Cohesion: 0.10
...
### Community 477 - "compare_runtimes.py"
Cohesion: 0.16
...
### Community 485 - "agent/workspace.py"
Cohesion: 0.15
Nodes (12): get_workspace_manager(), Exception, agent/workspace.py — Isolated workspace lifecycle management. Every agent…, Base class for all workspace errors., _validate_id(), WorkspaceAccessDeniedError, WorkspaceError, WorkspaceIDError (+4 more)
...
Cohesion: 0.15
Nodes (19): batch_compatibility(), get_compatibility_label(), get_hardware_profile(), get_hardware_profile_endpoint(), model_compatibility(), ModelCompatibilityLabel, Enum, get (+11 more)
...
### Community 491 - "TestAddConversationCacheBreakpoints"
Cohesion: 0.15
...
### Community 539 - "resolve_free_nvidia_brain"
Cohesion: 0.12
...
### Community 554 - "test_provider_failover_integration.py"
Cohesion: 0.15
...
### Community 555 - "test_repowise_intelligence.py"
Cohesion: 0.11
...
Cohesion: 0.13
Nodes (10): _iso_now(), _iso_offset_hours(), Enum, str, Open a workspace for resumption. Only READY or PAUSED workspaces may be…, Update *ws* status and persist the manifest., Update last_heartbeat timestamp and persist., Remove and recreate the tmp directory for *ws*. (+2 more)
...
### Community 572 - "test_omniroute_provider.py"
Cohesion: 0.12
...
### Community 578 - "test_agent_tools.py"
Cohesion: 0.19
...
Cohesion: 0.14
Nodes (8): _parse_iso(), Create, open, and lifecycle-manage per-job isolated workspaces. All workspaces…, Raise :exc:`WorkspaceAccessDeniedError` if *requesting_session_id* doesn't own…, Acquire the exclusive async lock for *ws*. Raises :exc:`WorkspaceLockError` if…, Remove expired workspaces that are cleanup_eligible and past cleanup_after.…, Return counts of workspaces by status (scans disk, sync). Call ``await…, WorkspaceManager, TestWorkspaceSecurityRegressions
...
### Community 617 - "_valid_login_state"
Cohesion: 0.24
...
### Community 631 - "AdminDigestRouterAuthTests"
Cohesion: 0.20
...
### Community 633 - "test_telegram_auto_approve.py"
Cohesion: 0.21
...
### Community 675 - "test_swarm_concurrency.py"
Cohesion: 0.15
...
### Community 680 - "register_user_research_tools"
Cohesion: 0.19
...
### Community 690 - "admin_digest_router.py"
Cohesion: 0.22
...
### Community 703 - "detect_hardware"
Cohesion: 0.14
Nodes (14): _detect_amd_gpus(), _detect_apple_silicon_gpu(), _detect_cpu(), detect_hardware(), _detect_intel_arc_gpu(), _detect_nvidia_gpus(), _detect_ram(), Return (model_name, physical_cores, logical_threads). (+6 more)

### Community 704 - "test_hardware.py"
Cohesion: 0.21
Nodes (6): _lookup_requirements(), ModelCompatibility, Any, Return the best-matching requirement spec for a model name., tests/test_hardware.py — Unit tests for the hardware detection module., TestModelRequirements
...
### Community 707 - "is_anthropic_base_url"
Cohesion: 0.18
...
### Community 710 - "_self_heal_ready"
Cohesion: 0.16
...
### Community 754 - "_is_bedrock_model_id"
Cohesion: 0.27
...
### Community 755 - "verify_service_token"
Cohesion: 0.22
...
### Community 759 - "_InMemoryBackend"
Cohesion: 0.18
...
### Community 771 - "PortfolioIntelligence"
Cohesion: 0.18
...
### Community 794 - "check_model_compatibility"
Cohesion: 0.36
...
### Community 802 - "TestDeprecationWarnings"
Cohesion: 0.17
...
### Community 815 - "make_isolated_workspace"
Cohesion: 0.33
...
### Community 859 - "mint_access_token"
Cohesion: 0.18
...
### Community 886 - "HardwareProfile"
Cohesion: 0.33
Nodes (3): GPUDevice, HardwareProfile, TestHardwareProfile
...
### Community 897 - "_FakeInner"
Cohesion: 0.22
Nodes (3): _FakeInner, Any, Stands in for MCPClient inside RenderMCPClient.

### Community 898 - "test_source_id_index_is_partial_not_sparse"
Cohesion: 0.22
...
### Community 907 - "WorkspaceEscapeError"
Cohesion: 0.22
...
### Community 951 - "DirectChatDoctor"
Cohesion: 0.32
...
### Community 956 - "CapacityAllocation"
Cohesion: 0.25
...
### Community 959 - "_brain_provider_status"
Cohesion: 0.25
...
### Community 1031 - "test_runtimes_health_endpoint.py"
Cohesion: 0.25
...
### Community 1033 - "test_seo_audit_pending_expiry.py"
Cohesion: 0.43
...
### Community 1045 - "github_oauth_callback"
Cohesion: 0.33
Nodes (7): github_oauth_callback(), _error(), _oauth_popup_html(), Tiny HTML page that fires postMessage to the opener then self-closes., GitHub redirects here after the user authorises (or denies) the OAuth App., serve_spa(), HTMLResponse
...
### Community 1070 - "Workspace Issues"
Cohesion: 0.29
...
### Community 1071 - "Model and Response Issues"
Cohesion: 0.29
...
### Community 1130 - "TestDirectChatAgentExecution"
Cohesion: 0.33
...
### Community 1131 - "TestWorkflowIntegration"
Cohesion: 0.33
...
### Community 1158 - "Documentation map"
Cohesion: 0.40
...
### Community 1162 - "Admin Dashboard Issues"
Cohesion: 0.40
...
### Community 1163 - "Agent API Issues"
Cohesion: 0.40
...
### Community 1164 - "Network and Tunnel Issues"
Cohesion: 0.40
...
### Community 1183 - ".get_bound_skills"
Cohesion: 0.40
...
### Community 1190 - "TestInternalAgentAdapterProviderChain"
Cohesion: 0.40
...
### Community 1195 - "TestModelRoleSeparation"
Cohesion: 0.40
...
### Community 1217 - "Claude Code Specific Issues"
Cohesion: 0.50
...
### Community 1222 - "_clean_director"
Cohesion: 0.50
...
### Community 1246 - "ControlUpdateBody"
Cohesion: 0.67
...
- **3548 isolated node(s):** `duplicate.sh script`, `heartbeat.sh script`, `redact_secrets.sh script`, `docker`, `RENDER_API_KEY` (+3543 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 14419 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **123 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `_fixture()` connect `_fixture` to `TaskStatus`, `models/company_graph.py`, `test_mcp_protocol_version.py`, `test_ping.py`, `test_runtimes_health_endpoint.py`, `brain_config.py`, `PolicyEngine`, `captured`, `test_llm_router_strategies.py`, `test_llm_router_queue_cache.py`, `Task`, `UsageEvent`, `register_webui`, `test_model_router.py`, `AgentRunner`, `Agency`, `E2BSandboxSession`, `test_failover_client_shared.py`, `TaskWorkflowService`, `services/background.py`, `test_llm_router_resilience.py`, `test_autonomy_status.py`, `AgentSwarm`, `test_brain_patch_service_token.py`, `ImprovementLoop`, `test_freebuff_bot.py`, `test_unit8_model_catalog.py`, `test_provider_failover_integration.py`, `test_governance_enforcement.py`, `direct_chat.py`, `UserRole`, `test_llm_router_gateway.py`, `validate_policy_text`, `client`, `services/seo_audit.py`, `LLMRouter`, `test_startup_warmup.py`, `test_company_api.py`, `WorkflowRun`, `test_monitor_lib.py`, `SelfHealingAgent`, `TokenBudget`, `get_task_store`, `CEOSupervisor`, `test_sam_livekit.py`, `ResearchTask`, `PrimeAgentAdapter`, `cost_tracker.py`, `test_autonomous_agency_e2e.py`, `TestClient`, `test_procedural_memory.py`, `e2e/test_browser.py`, `test_knowledge_sync.py`, `test_openclaw_endpoints.py`, `config.py`, `ArtifactStore`, `test_task_brain_preflight.py`, `KeyPool`, `clear_cooldowns`, `NotificationDispatcher`, `test_catalogue_probe.py`, `resolve_e2b_config`, `SQLiteStore`, `test_response_cache.py`, `test_telegram_auto_approve.py`, `loop.py`, `persist_plan_spec`, `test_local_brain_state.py`, `test_context_rulebook.py`, `TestChatHistoryStore`, `WorkflowBuildRequest`, `test_llm_router_e2e.py`, `test_refresh_agent_built_proof.py`, `_resolve_brain_provider`, `test_issue_intake.py`, `test_trend_watcher.py`, `reset_cache`, `AuditLog`, `test_new_features_e2e.py`, `WorkflowRun`, `FeatureMaturity`, `test_kimi_bridge_server.py`, `tests/conftest.py`, `run_task`, `ChatHistoryStore`, `_step`, `ApprovalStore`, `test_telegram_freebuff.py`, `test_webui_provider_priority.py`, `TestHarnessAdapter`, `controls_app`, `TestEstimateTokensForMessages`, `claim`, `test_agent_tool_governance.py`, `test_e2b_task_wiring.py`, `MCPClient`, `KeyStore`, `_clean_phases`, `test_e2b_data_flow.py`, `E2BAdapter`, `test_daily_2026_07_27.py`, `SchedulerStore`, `OllamaCircuitBreaker`, `_clean_director`, `test_changelog_check_workflow.py`, `_job_text`, `test_video_transcript.py`, `emit_chat_observation`, `_run_analyze`, `test_openclaw_gateway.py`, `sam`, `test_features_api.py`, `TrendWatcher`, `models/seo_audit.py`, `test_scheduler_hydration_bounded.py`, `_Budget`, `test_purge_backlog.py`, `self_healing.py`, `test_llm_router_disabled.py`, `filed`, `CompanyAgencyService`, `TestClient`, `test_telegram_webhook.py`, `test_control_plane_api.py`, `timedelta`, `OrchestratorQueue`, `ContextPruner`, `ai/router.py`, `test_rate_limiter.py`, `test_mcp_governance.py`, `test_persistent_memory.py`, `test_provider_enable_disable.py`, `test_sam_voice.py`, `SecurityScanner`, `test_agile_api.py`, `test_app_settings.py`, `test_nvidia_model_discovery.py`, `OutputFilter`, `_resolve_user_github_token`, `test_gh_brain_failover.py`, `test_provider_state_durability.py`, `test_spa_trailing_slash_redirect.py`, `test_task_clarification.py`, `CacheManager`, `OrchestratorCheckpointStore`, `TestNormalizeResponseFormat`, `test_platform_controls.py`, `session_retro.py`, `test_agency_fix.py`, `test_skill_registry_boot_refresh.py`, `analyze_page`, `_redact_for_notification`, `PolicyDecision`, `test_phase5_doctor.py`, `parse_event_stream`, `test_ceo_router.py`, `isolated_telegram_config`, `test_regression.py`, `test_agent_scripts_share_one_model_list.py`, `test_crispy_burn_in.py`, `TestTheWorkflowIsSafeAndReadOnly`, `test_executive_advisory_api.py`, `TestMongoGate`, `_Recorder`, `test_agent_free_brain.py`, `_undeclared`, `test_dashboard_cache.py`, `SeoCheckDefinition`, `ai/watchdog.py`, `TestSwarmRoleRouting`, `test_service_token.py`, `test_telegram_diag_endpoint.py`, `test_harness_spec.py`, `OperationalIncidentTracker`, `test_process_quick_note_workflow.py`, `_mock_provider_records`, `api_keys_for`, `test_v4_api.py`, `ClaudeCodeAdapter`, `get_ceo_dispatcher`, `test_voice_pipeline.py`, `test_real_mongo_round_trip`?**
  _High betweenness centrality (0.079) - this node is a cross-community bridge._
- **Why does `AgentRunner` connect `AgentRunner` to `backend/server.py`, `proxy.py`, `AdaptiveHalter`, `ContextPruner`, `PolicyEngine`, `test_agent_chat_integration.py`, `test_agent_free_brain.py`, `RuntimeHealth`, `UserMemoryStore`, `FreeBuffAgent`, `test_backend_server_features.py`, `E2BSandboxSession`, `test_failover_client_shared.py`, `ReactScratchpad`, `AgentSessionStore`, `TaskSpec`, `InternalAgentAdapter`, `TestDeprecationWarnings`, `TestInternalAgentAdapterProviderChain`, `test_agent_tool_governance.py`, `MCPClient`, `direct_chat.py`, `TestAgentLoopMCPIntegration`, `test_agent_runner.py`, `workflow_orchestrator.py`, `ContextManager`, `E2BAdapter`, `test_governance_api.py`, `WorkflowRun`, `test_daily_automation_2026_07_11.py`, `MultiAgentSwarm`, `StuckDetector`, `TokenBudget`, `WorkspaceTools`, `test_autonomous_agency_e2e.py`, `test_empirical_verify.py`, `get_ceo_dispatcher`, `coordinator.py`, `CEODispatcher`, `LocalWorkspace`, `loop.py`, `GitHubTools`?**
  _High betweenness centrality (0.048) - this node is a cross-community bridge._
- **Why does `ProviderRouter` connect `ProviderRouter` to `backend/server.py`, `system_instruction`, `TestAnthropicPayloadStructuredOutput`, `ProviderConfig`, `ai/router.py`, `register_webui`, `test_bedrock_provider.py`, `test_all_providers_discovery.py`, `TestLegacyRouterServerFallback`, `test_provider_failover_integration.py`, `direct_chat.py`, `is_anthropic_base_url`, `implement_agent.py`, `failover_client.py`, `test_bedrock_live.py`, `Provider`, `test_provider_router.py`, `kimi_bridge_provider_config`, `test_colibri_provider.py`, `_payload`, `TestLegacyRouterCacheTTL`, `test_anthropic_refusal_fallback.py`, `TestRouterIntegration`, `resolve_provider_for`, `clear_cooldowns`, `NvidiaProvider`, `TrafficDirector`?**
